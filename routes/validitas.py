    ...
    if request.method == 'POST':
        ...
        if action == 'search':
            ...
            if not df_match.empty:
                ...
                # Ambil baris PERTAMA (seperti kode awal)
                row = df_match.iloc[0]

                # =====================================================
                # LOAD IZIN EDAR DARI result_validitas  (ambil terbaru)
                # =====================================================
                izin_edar_value = ""
                try:
                    conn = mysql.connector.connect(**MYSQL_CONFIG)
                    ...
                    # Normalisasi barcode yang benar
                    barcode_for_search = str(row['barcode']).strip()
                    barcode_for_search = barcode_for_search.lstrip('0')
                    barcode_for_search = barcode_for_search.zfill(13)

                    query_izin = """
                        SELECT `IZIN EDAR`
                        FROM result_validitas
                        WHERE BARCODE = %s
                            AND `IZIN EDAR` IS NOT NULL
                            AND `IZIN EDAR` <> ''
                        ORDER BY id DESC
                        LIMIT 1
                    """
                    cursor.execute(query_izin, (barcode_for_search,))
                    izin_row = cursor.fetchone()

                    if izin_row and izin_row[0]:
                        izin_edar_value = izin_row[0]

                    cursor.close()
                    conn.close()

                except Error as e:
                    logger.error(f"❌ ERROR LOAD IZIN EDAR: {e}")

                # =====================================================
                # SET PRODUK (TERMASUK IZIN EDAR)
                # =====================================================
                selected_product = {
            ...
            try:
                ...
                if gc:
                    try:
                        ...
                        message += f' di (<a href="{gs_url}" target="_blank">Google Spreadsheet</a>)'
