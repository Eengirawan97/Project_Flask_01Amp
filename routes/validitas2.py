...
def input_page():
    ...
    # Ambil semua nilai input form (atas & bawah) agar tidak hilang
    no_gang = request.form.get('no_gang_top', '')
    minggu_ke = request.form.get('minggu_ke', '')
    bulan_tahun = request.form.get('bulan_tahun', '')
    expired = request.form.get('expired', '')
    izin_edar = request.form.get('izin_edar', '')
    ...
    if request.method == 'POST':
        ...
        # =========================================================
            ...
            barcode = request.form.get('barcode', '').strip().zfill(13)
            ...
            no_gang = request.form.get('no_gang_top', '').strip()
            minggu_ke = request.form.get('minggu_ke', '').strip()
            bulan_tahun = request.form.get('bulan_tahun', '').strip()
            ...
            try:
                ...
                insert_query = """
                    ...
                    (CATEGORY, prod_code, BARCODE, nama_produk, EXPIRED, `IZIN EDAR`,
                     entry_date, nomor_gang, minggu_ke, bulan_tahun)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                """

                cursor.execute(insert_query, (
                    kategori, prodcode, barcode, nama_produk,
                    expired_db, izin_edar, tanggal,
                    no_gang, minggu_ke, bulan_tahun
                ))
